...
func (s *Service) Checkout(ctx context.Context, stayID, actorID, requestID string) (domain.OperationalTask, error) {
	storeContext := ctx
	if ctx.Err() != nil {
		storeContext = context.Background()
	}
	stay, err := s.store.GetStay(storeContext, stayID)
	if err != nil {
	...
	if err := s.store.CheckoutAndCreateCleaning(storeContext, updated, task, event); err != nil {
