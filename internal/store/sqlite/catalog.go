...
func (s *Store) UpdatePropertyStatus(ctx context.Context, id string, from, to domain.PropertyStatus, version int64, now time.Time) error {
	result, err := s.db.ExecContext(ctx, `UPDATE properties SET status=?,version=version+1,updated_at=? WHERE id=? AND status=? AND version=?`, to, encodeTime(now), id, from, version)
	if err != nil {
